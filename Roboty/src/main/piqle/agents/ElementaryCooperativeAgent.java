...
import piqle.environment.ExtendedStateWithActions;
import piqle.environment.NullableAction;
import piqle.environment.Filter;
import piqle.environment.IEnvironment;
import piqle.environment.IState;

/**
* We want to have piqle.agents (in a multi-agent frame) that are able to know
...
* piqle.environment's state (through a filter), and the last actions of its
* neighbours.
* As we must also define the initial state, we need the notion of null action 
* (when no piqle.agents has yet moved).
* 
*   
* @author decomite
*
...
extends ElementaryAgent {
	...
	/**
	 * @param s  the local piqle.environment (which defines possible actions)
